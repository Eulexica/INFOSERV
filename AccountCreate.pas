...
begin
   try
//      InsightiTrackWatcher.WriteLog('attempting financial transaction start');
 //     InsightiTrackWatcher.uniInsight.StartTransaction;
//      InsightiTrackWatcher.WriteLog('financial start transaction');
      try
...
//            ParamByName('descr').AsString := ADescr;
            InsightiTrackWatcher.WriteLog('checking existence of invoice - Creditor: '+ IntToStr(ACreditor) +' REFNO:'+ AOrderID);
            Open;
            if (EOF = True) then
            begin
               InsightiTrackWatcher.WriteLog('attempting to save invoice: '+ IntToStr(ACreditor) +' REFNO:'+ AOrderID);
               TaxCode := 'GST';
